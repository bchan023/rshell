...
//helped
CommandList::CommandList(vector<string> &inputSplit, unsigned &cur,int layer, int &flag){
    std::regex quoteReg{"\""};
    bool endPar = false;
    this -> IsConnector = false;
    vector<Base*> userCall;
    vector<Base*> userInputs;
    vector<std::vector<Base*> > toBreak;
    vector<std::string> passInArg;
    string quotes;
    bool isTest = false;
    bool inQuotes = false;
    while (cur < inputSplit.size()){
        ...
        if (findLoc != (std::string::npos)){
            if (inQuotes){
                inQuotes = false;
                recived = regex_replace (recived,quoteReg,string(""));
                quotes += " " + recived;
...
string CommandList:: colonget(std::string input,bool &endPar){
    input = input.substr(0,input.size() -1);
    if (input.back() == ')'){
        endPar = true;
        return paranthesesget(input);
    }
    return input;
}
string CommandList::paranthesesget(std::string input){
    input = input.substr(0,input.size() -1);
    if (input.back() == ';'){
            input = input.substr(0,input.size() -1);
            return input;
